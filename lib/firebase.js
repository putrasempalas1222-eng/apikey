...
async function bootstrapProfile(user) {
  const ref = getDatabase(firebaseApp()).ref(`users/${user.uid}/profile`)
  const now = Date.now()
  let changed = false
  const result = await ref.transaction((profile) => {
    ...
    const plan = record.plan === "plus" ? "plus" : "free"
    const defaultLimit = plan === "plus" ? 500000 : 100000
    ...
    if (!(Number.isFinite(record.quotaResetsAt) && record.quotaResetsAt > 0)) {
      record.quotaResetsAt = now + (plan === "plus" ? 7 : 30) * 24 * 60 * 60 * 1000
      modified = true
    }
    if (!modified) return
  ...
  return { changed, profile: result.snapshot.val() }
}
